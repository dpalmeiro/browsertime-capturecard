...
class Measure {
  constructor(
    ...
    this.compositorVideos = compositorVideos;
    this.videos = videos;
  ...
  async start(urlOrAlias, optionalAlias) {
    ...
    if (this.options.firefox.compositorVideo) {
      await ks.sendCombination(['control', 'shift', '4']);
      //let driver = this.browser.getDriver();
      //const oldContext = this.browser.getDriver().getContext();
      //await driver.setContext("chrome");
      //const script = `windowUtils.setCompositionRecording(1); console.log("turned it on");`;
    ...
    if (this.options.firefox.compositorVideo ||
          (this.recordVideo && !this.options.videoParams.debug)) {
  ...
  async collect(testedStartUrl) {
    ...
    if (this.options.firefox.compositorVideo) {
      ...
      //const oldContext = this.browser.getDriver().getContext();
      //await driver.setContext("chrome");
      //const script = `windowUtils.setCompositionRecording(0); console.log("turned it off");`;
      //await driver.executeScript(script);
      //await driver.setContext(oldContext);
      // We need this delay so the browser has enough time to write out all the PNG files.
      // TODO: should be some kind of "I'm done writing" promise from the browser instead.
      await delay(30000);

      let globOptions = {};
      globOptions.absolute = true;
      let videoDir = glob.sync("windowrecording-*", globOptions).pop();
      const execa = require('execa');
      await execa('powershell.exe', ['C:\\benchmark\\AnalyzeWindowRecording.ps1',videoDir]);
      this.compositorVideos.push(videoDir);
    }
