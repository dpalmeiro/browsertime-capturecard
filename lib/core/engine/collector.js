...
class Collector {
  ...
  async perIteration(allData, index) {
    for (let data of allData) {
      ...
      if (data.compositorVisualMetrics) {
        if (this.options.iterations > 1) {
          log.info(
            `CompositorVisualMetrics FirstVisualChange: ${
              data.compositorVisualMetrics.FirstVisualChange
            } SpeedIndex: ${
              data.compositorVisualMetrics.SpeedIndex
            } VisualComplete85: ${
              data.compositorVisualMetrics.VisualComplete85
            } LastVisualChange: ${data.compositorVisualMetrics.LastVisualChange}`
          );
        }
        results.compositorVisualMetrics.push(data.compositorVisualMetrics);
        statistics.addDeep({
          compositorVisualMetrics: data.compositorVisualMetrics
        });
      }
