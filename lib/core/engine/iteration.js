...
const getVideoMetrics = require('../../video/postprocessing/visualmetrics/single/getVideoMetrics');
const fs = require('fs');
...
class Iteration {
  ...
  async run(navigationScript, index) {
    ...
    try {
      ...
      if (options.firefox.compositorVideo) {
        let i = 0;
        for (let myVideoDir of compositorVideos) {
          try {
            let imageDir = myVideoDir + '/images' + '/' + i;
            if (!fs.existsSync(imageDir)) {
              fs.mkdirSync(myVideoDir + '/images');
              fs.mkdirSync(imageDir, { recursive: true });
            }
            const videoMetrics = await getVideoMetrics(
              myVideoDir,
              myVideoDir + '\\video.mp4',
              i,
              null,
              this.storageManager,
              this.options
            );
            result[i].compositorVisualMetrics = videoMetrics.visualMetrics;
            var rimraf = require("rimraf");
            rimraf(myVideoDir, function () {});
            i++;
          } catch (e) {
