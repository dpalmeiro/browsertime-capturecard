  ...
  harFromEvents(events, options) {
    function populateEntryFromResponse(entry, response) {
      ...
      if (timing) {
        ...
        // follow same naming standard as WPT
        entry._was_push = timing.pushEnd - timing.pushStart > 0;
        ...
        if (!entry._servedFromCache) {
          ...
          if (response.connectionReused && !entry._mainRequest && isHttp1x(response.protocol)) {
            ...
            let stalledTimeDiff = ((entry._totalRequestTime - entry.time) / 1000);
            entry._stalledTimeDiff = stalledTimeDiff;
            let newStartDateTime = entry._wallTime + stalledTimeDiff;
            entry.startedDateTime = moment.unix(newStartDateTime).toISOString();  
          }
        }
      } else {
