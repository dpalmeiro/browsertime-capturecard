...
module.exports = {
  ...
  addExtrasToHAR(index, harPage, visualMetricsData, timings, cpu, options) {
    ...
    if (timings && timings.userTimings && timings.userTimings.marks) {
      harPage._userTimings = {};
      harPage._userTimings.marks = {};
      const maxUserTimings = 3;
      let addedTimings = 0;
      for (let mark of timings.userTimings.marks) {
        if (addedTimings < maxUserTimings) {
          // We don't add all user metrics to the timings becasue
          // that pollutes the HAR viewer
          harPageTimings['_' + mark.name] = mark.startTime;
          addedTimings++;
        }
        harPage._userTimings.marks[mark.name] = mark.startTime;
      }
    }
    if (timings && timings.navigationTiming) {
      harPage._navigationTimings = timings.navigationTiming;
    }
