...
async function copyFromRemote(outputPath) {
  return spawn('scp', ['denis@192.168.0.3:C:\\Users\\Denis\\Desktop\\output.mp4', outputPath], {shell:true});
  //return spawn('scp', ['192.168.0.3:/tmp/output.mp4', outputPath], {shell:true});
}
...
module.exports = class X11Recorder {
  ...
  async stop(destination) {
    ...
    try {
      await copyFromRemote(destination);
